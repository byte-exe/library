...
class Apps extends CI_Controller {
	...
	public function nama_perpus() {
		...
		$kepsek_nip	= addslashes($this->input->post('kepsek_nip'));
		...
		if ($mau_ke == "simpan") {
			if ($_FILES['logo']['name'] != "") {	
				if ($this->upload->do_upload('logo')) {
					...
					$this->db->query("UPDATE r_config SET nama = '$nama', alamat = '$alamat', logo = '".$up_data['file_name']."', kepsek = '$kepsek', kepsek_nip = '$kepsek_nip', petugas = '$petugas', petugas_nip = '$petugas_nip', profil = '$profil' WHERE id = '$idp'");
				...
				$this->db->query("UPDATE r_config SET nama = '$nama', alamat = '$alamat', kepsek = '$kepsek', kepsek_nip = '$kepsek_nip', petugas = '$petugas', petugas_nip = '$petugas_nip', profil = '$profil' WHERE id = '$idp'");
	...
	public function l_peminjam() {
		...
		$m['p_tahunini']	= $this->db->query("SELECT a.*, c.nama nmanggota, b.judul FROM t_trans a
											LEFT JOIN t_buku b ON a.id_buku = b.id
											LEFT JOIN t_anggota c ON a.id_anggota = c.id ")->result();
