...
<br>
	...
	<table style="width: 50%"  class="table table-condensed">
		...
		<?php 
		$c_jen_hi 	= $this->db->query("SELECT SUM(IF( jenis =  'Siswa', 1, 0 ) ) AS siswa, SUM( IF( jenis =  'Guru', 1, 0 ) ) AS Guru FROM t_pengunjung  WHERE LEFT(tgl, 10) = DATE(NOW()) ")->result();
		
		foreach($c_jen_hi as $cjh) {
		?>
		<tr>
			<td style="text-align: center">1</td>
			<td style="text-align: center"><?=$cjh->siswa?></td>
			<td style="text-align: center"><?=$cjh->Guru?></td>
			<td style="text-align: center"><?=($cjh->siswa + $cjh->Guru)?> orang</td>
		</tr>	
		<?php
		}
		?>
	...
	<table style="width: 50%"  class="table table-condensed">
		...
		<?php 
		$c_jen_hi 	= $this->db->query("SELECT SUM(IF( jenis =  'Siswa', 1, 0 ) ) AS siswa, SUM( IF( jenis =  'Guru', 1, 0 ) ) AS guru FROM t_pengunjung  WHERE MID(tgl, 6, 2) = MONTH(NOW()) ")->result();
