	...
	<table style="width: 50%; border-collapse: collapse" border="1" class="table table-condensed">
		...
		<?php 
		$c_jen_hi 	= $this->db->query("SELECT SUM(IF( jenis =  'Siswa', 1, 0 ) ) AS siswa, SUM( IF( jenis =  'Guru', 1, 0 ) ) AS guru FROM t_pengunjung  WHERE MID(tgl, 6, 2) = '$bulan' ")->result();
		
		foreach($c_jen_hi as $cjh) {
		?>
		<tr>
			<td style="text-align: center">1</td>
			<td style="text-align: center"><?=_null($cjh->siswa)?></td>
			<td style="text-align: center"><?=_null($cjh->guru)?></td>
			<td style="text-align: center"><?=_null(($cjh->siswa + $cjh->guru))?></td>
		</tr>	
		<?php
		}
		?>
